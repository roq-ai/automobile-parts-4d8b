...
export function OrderItemListPage(props: OrderItemListPageProps) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('order', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('part', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'part',
          header: 'Part',
          accessorKey: 'part',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/parts/view/${record.part?.id}`}>
              {record.part?.name}
            </Link>
          ),
        }
