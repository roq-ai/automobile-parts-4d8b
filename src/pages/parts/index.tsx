...
import { getParts, deletePartById } from 'apiSdk/parts';
import { PartInterface } from 'interfaces/part';

type ColumnType = ColumnDef<PartInterface, unknown>;
...
export function PartListPage(props: PartListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getParts({
        relations: ['supplier', 'order_item.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PartInterface>>(
    () => `/parts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PartInterface) => {
    if (hasAccess('part', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/parts/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'stock_level', header: 'stock_level', accessorKey: 'stock_level' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/parts/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('part', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/parts/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('part', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="part" />
        </Flex>

        {hasAccess('part', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/parts/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
