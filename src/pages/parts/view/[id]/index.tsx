...
import { getPartById } from 'apiSdk/parts';
import { PartInterface } from 'interfaces/part';
import { OrderItemListPage } from 'pages/order-items';

function PartViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<PartInterface>(
    () => (id ? `/parts/${id}` : null),
    () =>
      getPartById(id, {
        relations: ['supplier'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('part', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/parts/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Stock Level" text={data?.stock_level} />
